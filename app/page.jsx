...
function NumBadge({ num, active = true, small = false }) {
  return (
    <span className={`${active ? 'num active' : 'num'} ${small ? 'small' : ''}`}>
    ...
    <main className="page">
      <style jsx global>{`
        *{box-sizing:border-box}
        ...
        .special{background:#f59e0b!important;color:#111827!important}
        ...
        .good{color:#4ade80;font-weight:900}.mid{color:#facc15;font-weight:900}.bad{color:#fb7185;font-weight:900}
      ...
      <div className="wrap">
        <section className="hero">
          <div className="card">
            ...
            <p className="muted">
              自动抓取最近200期开奖，只统计最后一个号码，也就是特码。按 20 / 50 / 100 / 200 期分别计算出现次数、出现率和当前遗漏，并自动推荐综合热度最高的10个号码。
            </p>
          ...
          <div className="card">
            ...
            <div className="latest">
              {(latest?.numbers || []).slice(0, 6).map((num, index) => <NumBadge key={`${num}-${index}`} num={num} />)}
              <strong>+</strong>
              {latest?.specialNumber && <NumBadge num={latest.specialNumber} active small={false} />}
            </div>
          ...
          <div>
            ...
            <div className="card">
              ...
              <div className="input-grid">
                <div><div className="muted">单号金额</div><input className="input" value={betAmount} onChange={(e) => setBetAmount(e.target.value)} /></div>
                <div><div className="muted">赔率</div><input className="input" value={odds} onChange={(e) => setOdds(e.target.value)} /></div>
                <div><div className="muted">购买号码数量</div><input className="input" value={buyCount} onChange={(e) => setBuyCount(e.target.value)} /></div>
              </div>
            ...
            <div className="card">
              ...
              <div className="tabbar">
                {WINDOWS.map((size) => (
                  <button key={size} className={windowSize === size ? 'tab active' : 'tab'} onClick={() => setWindowSize(size)}>近{size}期</button>
                ))}
              </div>

              <table>
                <thead><tr><th>排名</th><th>号码</th><th>出现次数</th><th>出现率</th><th>当前遗漏</th><th>热度</th><th>最近出现</th></tr></thead>
                <tbody>
                  {rank.map((item, index) => (
                    <tr key={item.num}>
                ...
                <div className="history-row" key={item.expect}>
                  <div>
                    ...
                    <div className="history-nums">
                      {item.numbers.slice(0, 6).map((num, index) => <NumBadge key={`${item.expect}-${num}-${index}`} num={num} small />)}
                      <strong>+</strong>
                      <span className="num active small">{item.specialCode}</span>
                    </div>
                  </div>
                  <div className={recommendSet.has(item.specialNumber) ? 'good' : 'bad'}>{recommendSet.has(item.specialNumber) ? '推荐命中' : '未命中'}</div>
                </div>
              ))}
            </div>
          </aside>
